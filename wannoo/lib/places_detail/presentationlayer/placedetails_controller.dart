...
final Rx<PlacedetailsModel> demoData = PlacedetailsModel().obs;
  ...
  void getPlaceData(String placeId) {
    getPlaceDetailsUseCase.execute(PlacesDetailRequest(id: placeId)).then((response) {
      // Map response data to a single PlacedetailsModel instance
      if (response != null) {
        imagePaths.assignAll(
            response.tourImages?.map((image) => ImageModel(
        ...
        demoData.value = PlacedetailsModel(
          ...
          id: response.id ?? 0,
          reviews: 120,
          images: response.tourImages?.map((image) => ImageModel(
          ...
          faq: response.faq?.map((faq) => Faq(
            id: faq.id,              // Adjust the key names according to your map structure
